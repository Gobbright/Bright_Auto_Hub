...
export default function BlogDetailPage({ data, slug, heroImage }) {
  const tag = data.tags?.[0] || 'AUTOMOTIVE JOURNAL'
  ...
  return <MarketplaceShell>
    <main className='blog-single-page min-h-screen overflow-hidden bg-[#f6f3ee] text-[#18202d]'>
      ...
      <section className='mx-auto my-[clamp(38px,5vw,70px)] flex w-[min(1180px,calc(100%-36px))] items-center justify-between gap-6 rounded-[22px] border border-red-100 bg-[#fff0f2] p-[clamp(28px,4vw,46px)] max-md:flex-col max-md:items-start'><div><small className='text-[9px] font-black tracking-[.14em] text-[#e5091a]'>SMARTER JOURNEYS START HERE</small><h2 className='mb-2 mt-2 max-w-[650px] text-[clamp(27px,3vw,42px)] font-bold leading-tight tracking-[-.04em]'>Ready to make your next vehicle decision?</h2><p className='m-0 text-xs text-slate-500'>Share your requirement and our experts will help with the next step.</p></div><Link className='shrink-0 rounded-lg bg-[#e5091a] px-5 py-3 text-[11px] font-extrabold text-white shadow-lg transition hover:-translate-y-0.5 hover:bg-[#ff3042] max-md:w-full max-md:text-center' to={enquiryUrl}>Enquire with an expert →</Link></section>
    </main>
  </MarketplaceShell>
}
